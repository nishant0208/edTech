...
function parseTeacherStatus(statusString: string | undefined): TeacherStatus | undefined {
  if (!statusString) return undefined;
  const upperStatus = statusString.toUpperCase();
  if (Object.values(TeacherStatus).includes(upperStatus as TeacherStatus)) {
    return upperStatus as TeacherStatus;
  }
...
router.post('/bulk-upload', async (req: any, res: any) => {
  const teacherRecords: any[] = req.body;
    ...
    try {
      ...
      if (!user) {
        ...
        user = await prisma.user.create({
          data: {
            ...
            role: UserRole.TEACHER, // Assign correct role
            isActive: true,
          },
        });
      ...
      const newTeacher = await prisma.teacher.create({
        data: {
          ...
          status: parseTeacherStatus(record.status) || TeacherStatus.ACTIVE, // Default to ACTIVE
      ...
      if (record.classes_taught_names) {
        ...
        for (const className of classNames) {
          const teacherClass = await prisma.class.findUnique({ where: { name: className } });
          if (!teacherClass) {
